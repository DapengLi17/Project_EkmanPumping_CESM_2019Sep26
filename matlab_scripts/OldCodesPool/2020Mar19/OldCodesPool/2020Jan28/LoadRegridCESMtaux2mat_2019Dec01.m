...
% descrip: matlab scripts load taux from CESM nc files, 
...
% --- input/output files ---
InDirTAUX = '../raw_data/CESMncFilesKEFromAgartha_ZhangQiuying/TAUX';
Outfile1 = ['../data_after_manipulation/CESMtaux_KE_',date_str,'.mat'];
Pic1 = ['../pics/CheckRegrid/CheckRegridTAUX_',date_str,'.png'];
% --------------------------
...
%% === load data ===
[~,ulat,ulon,taux_raw] = LoadCESMKEncFiles3DVarFunc(InDirTAUX,TAUX_Str);
% ===================
...
taux_unitconvt = taux_raw.*0.1; % [N/m²]
...
for iDay = 1:size(taux_unitconvt,3) % loop through time
   taux_rot(:,:,iDay) = rot90(taux_unitconvt(:,:,iDay));
end

[Lon,Lat]=meshgrid(Lon_vec,Lat_vec);

for iDay=1:size(taux_rot,3) % loop through time
  iDay
...
f1=figure; % check rotation
  ...
  plt_indx = 374;
  taux_plt = taux_unitconvt(:,:,plt_indx);
  taux_pltmin=nanmin(taux_plt(:));
  taux_pltmax=nanmax(taux_plt(:));  
  
 subplot(2,2,1) % raw data
  pcolor(ulon,ulat,taux_unitconvt(:,:,plt_indx));shading interp;
  caxis([taux_pltmin taux_pltmax]);hc=colorbar;title(hc,'[N/m2]')
  title(['raw CESM taux, time: ',datestr(jultime(plt_indx))]);
  
 subplot(2,2,2) % data after rotation
  pcolor(ulon_rot,ulat_rot,taux_rot(:,:,plt_indx));shading interp;
  caxis([taux_pltmin taux_pltmax]);hc=colorbar;title(hc,'[N/m2]')
  title(['rotated CESM taux, time: ',datestr(jultime(plt_indx))]);
  
 subplot(2,2,3) % data after rotation and interpolation
  pcolor(Lon,Lat,taux(:,:,plt_indx));shading interp;
  caxis([taux_pltmin taux_pltmax]);hc=colorbar;title(hc,'[N/m2]')
  title(['rotated and interpolated CESM taux,  time: ', ...
      datestr(jultime(plt_indx))]);
 
 subplot(2,2,4) % time series
  taux_llav = squeeze(nanmean(nanmean(abs(taux),1),2));
  taux_llmax = squeeze(nanmax(nanmax(abs(taux),[],1),[],2));  
  h1=plot(jultime,taux_llav,'b');hold on;
  h2=plot(jultime,taux_llmax,'r');grid on;
    legend([h1,h2],{'lat-lon av taux','lat-lon max taux'});
    set(gca,'ylim',[0 1]);ylabel('N/m2');datetick('x','mmm/yy');
    title(['rotated and interpolated CESM taux time series', ...
        datestr(jultime(plt_indx))]);  
% ==============
...
header = ['This file was created by DL on ',date_str,' via code ', ...
    'ConvertCESMtauxy2mat_2019Dec01.m, units of taux is ', ...
    'converted to N/m2.'];
