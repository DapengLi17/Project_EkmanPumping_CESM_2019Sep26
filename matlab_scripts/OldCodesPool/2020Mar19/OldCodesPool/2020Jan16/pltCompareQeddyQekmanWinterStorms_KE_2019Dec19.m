...
% descrip: matlab scripts plot Qeddy 50m during winter without storms 
% 1) CESM model output
...
pic1 = ['../pics/CompareQeddyQekmanWinterStorms_KE_',date_str,'.png'];
...
%   IndxSea{1} = [IndxMon{6};IndxMon{7}; ...
%       IndxMon{8}]; % Winter season (Jing et al. 2019)
...
indx_ns = find(tau_llav_wt<tau_st_limt); % length(indx_ns) = 284
...
% kesai_ns = kesai_wt(:,:,indx_ns);
% kesai_nsd_ns = nanstd(kesai_ns,0,3);

Q_li_ns = Q_li_wt(:,:,indx_ns);
Q_li_ns_av = nanmean(Q_li_ns,3);

Q_nl_ns = Q_nl_wt(:,:,indx_ns);
Q_nl_ns_av = nanmean(Q_nl_ns,3);

Q_df_ns = Q_nl_ns - Q_li_ns;
Q_df_ns_av = nanmean(Q_df_ns,3);

Q_ce_ns = Q_ce_wt(:,:,indx_ns);
Q_ce_ns_av = nanmean(Q_ce_ns,3);

% --- Ro > Ro std ---
indxRo = find(Ro_wt_std>Ro_std_limit);length(indxRo)
w_li_ns_Ro = w_li_ns_av(indxRo);
w_nl_ns_Ro = w_nl_ns_av(indxRo); 
w_ce_ns_Ro = w_ce_ns_av(indxRo); 
Q_li_ns_Ro = Q_li_ns_av(indxRo);
Q_nl_ns_Ro = Q_nl_ns_av(indxRo); % nanmean(Q_nl_ns_Ro) = 21.4182
Q_ce_ns_Ro = Q_ce_ns_av(indxRo); % nanmean(Q_ce_ns_Ro) = 163.3431
...
% --- Ro > Ro std ---
w_li_st_Ro = w_li_st_av(indxRo);
w_nl_st_Ro = w_nl_st_av(indxRo); 
w_ce_st_Ro = w_ce_st_av(indxRo); 
Q_li_st_Ro = Q_li_st_av(indxRo);
Q_nl_st_Ro = Q_nl_st_av(indxRo); % nanmean(Q_nl_st_Ro) = 41.8167
Q_ce_st_Ro = Q_ce_st_av(indxRo); % nanmean(Q_ce_st_Ro) = 211.2867

% (nanmean(Q_nl_st_Ro)-nanmean(Q_nl_ns_Ro))./ ...
%     (nanmean(Q_ce_st_Ro)-nanmean(Q_ce_ns_Ro)) = 0.44
%%
...
bQ_nl2li = robustfit(Q_nl_st_Ro-Q_nl_ns_Ro,Q_ce_st_Ro-Q_ce_ns_Ro); 
% y = 30.0280 + 0.6471*x
...
indxNoNaN_Qnl = find(~isnan(Q_nl_st_Ro-Q_nl_ns_Ro));
indxNoNaN_Qce = find(~isnan(Q_ce_st_Ro-Q_ce_ns_Ro));
indxNoNaN_Q = intersect(indxNoNaN_Qnl,indxNoNaN_Qce);

rQ_nl2ce = corr2(Q_nl_st_Ro(indxNoNaN_Q)-Q_nl_ns_Ro(indxNoNaN_Q), ...
    Q_ce_st_Ro(indxNoNaN_Q)-Q_ce_ns_Ro(indxNoNaN_Q)); % rQ_nl2ce=0.2057

% nanmean(Q_li_ns_av(indxRo)) = 23.3479
% nanmean(Q_nl_ns_av(indxRo)) = 40.9070
% nanmean(Q_ce_ns_av(indxRo)) = 150.3891
...
f1=figure('Renderer','painters'); % composite av
  ...
  % set pic size
  pic1_size=[20 10]; % pic size: unit [inch] 3.155 inch is the width(length in x direction) of JPO 1 column pic, height varies according to your needs
  set(f1,'Units','inches','Position',[5,5,pic1_size]);
  font_size=12;
  Q_lim = [-200 200];
 
% ~~~ generate subplot position ~~~  
  row_num=2;col_num=3;margin_left=0.06;
...
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 ...
 subplot('position',sbplt_posit(1,:));
  ...
  [c,h]=contour(Lon,Lat,Ro_wt_std,[Ro_std_limit Ro_std_limit],'k');
    ...
    text(132,27.5,'a','fontsize',font_size);
    title([num2str(length(indx_st)),' storm days av Q_{cesm}  depth: ', ...
        num2str(zT_raw(indxZ)),' m']) 

 subplot('position',sbplt_posit(2,:));
  pcolor(Lon,Lat,Q_ce_ns_av);shading interp;
    caxis(Q_lim.*2);polarmap(64,1);hc=colorbar;title(hc,'W/m^2'); 
      hold on;
  [c,h]=contour(Lon,Lat,Ro_wt_std,[Ro_std_limit Ro_std_limit],'k');
    set(h,'LineWidth',1.5);
    ...
    text(132,27.5,'b','fontsize',font_size);
    title([num2str(length(indx_ns)),' no-storm days av Q_{cesm}  depth: ' ...
        ,num2str(zT_raw(indxZ)),' m'])
   
 subplot('position',sbplt_posit(3,:))
  pcolor(Lon,Lat,Q_ce_st_av-Q_ce_ns_av);shading interp;
    caxis(Q_lim.*2);polarmap(64,1);hc=colorbar;title(hc,'W/m^2');  
      hold on;
  [c,h]=contour(Lon,Lat,Ro_wt_std,[Ro_std_limit Ro_std_limit],'k');
    set(h,'LineWidth',1.5);
    set(gca,'xlim',[130 170],'ylim',[26.5 45.5],'ytick',[30:5:45], ...
        'xticklabel',[],'yticklabel',[],'fontsize',font_size);
    text(132,27.5,'c','fontsize',font_size);
    title(['Q_{cesm}(storms) - Q_{cesm}(no storms)  depth: ', ...
        num2str(zT_raw(indxZ)),' m']);
 ...
 subplot('position',sbplt_posit(4,:))
  ...
  [c,h]=contour(Lon,Lat,Ro_wt_std,[Ro_std_limit Ro_std_limit],'k');
    ...
    ylabel('Lat','fontsize',font_size);
    text(132,27.5,'d','fontsize',font_size);
    title([num2str(length(indx_st)),' storm days av Q_{\xi}  depth: ', ...
        num2str(zT_raw(indxZ)),' m']) 

 subplot('position',sbplt_posit(5,:))
  pcolor(Lon,Lat,Q_nl_ns_av);shading interp;
    caxis(Q_lim);polarmap(64,1);hc=colorbar;title(hc,'W/m^2'); 
      hold on;
  [c,h]=contour(Lon,Lat,Ro_wt_std,[Ro_std_limit Ro_std_limit],'k');
    set(h,'LineWidth',1.5);
    ...
    xlabel('Lon','fontsize',font_size);
    text(132,27.5,'e','fontsize',font_size);
    title([num2str(length(indx_ns)),' no-storm days av Q_{\xi}  depth: ', ...
        num2str(zT_raw(indxZ)),' m']);

 subplot('position',sbplt_posit(6,:))
  pcolor(Lon,Lat,Q_nl_st_av-Q_nl_ns_av);shading interp;
    caxis(Q_lim);polarmap(64,1);hc=colorbar;title(hc,'W/m^2'); 
      hold on;
  [c,h]=contour(Lon,Lat,Ro_wt_std,[Ro_std_limit Ro_std_limit],'k');
    set(h,'LineWidth',1.5);
    set(gca,'xlim',[130 170],'ylim',[26.5 45.5],'ytick',[30:5:45], ...
        'yticklabel',[],'fontsize',font_size);
    xlabel('Lon','fontsize',font_size);
    text(132,27.5,'f','fontsize',font_size);
    title(['Q_{\xi}(storms) - Q_{\xi}(no storms)  depth: ', ...
        num2str(zT_raw(indxZ)),' m'])   
...
%   
%   subplot('position',sbplt_posit(1,:))
%    plot(Q_ce_st_Ro-Q_ce_ns_Ro,Q_nl_st_Ro,'b*');hold on;
...
%   
%   subplot('position',sbplt_posit(2,:))
%    plot(w_nl_ns_Ro,w_ce_ns_Ro,'b*');hold on;
...
% 
%   subplot('position',sbplt_posit(3,:))
%    plot(Q_nl_ns_Ro,Q_li_ns_Ro,'b*');hold on;
...
%     
%   subplot('position',sbplt_posit(4,:))
%    plot(Q_nl_ns_Ro,Q_ce_ns_Ro,'b*');hold on;
