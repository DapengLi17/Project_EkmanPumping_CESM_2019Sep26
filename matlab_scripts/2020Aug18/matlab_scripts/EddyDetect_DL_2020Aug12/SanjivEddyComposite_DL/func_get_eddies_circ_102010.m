...
%                  [shape_test, minrad_test, maxrad_test, ocean_test, minpts_test, sign_test, nan_test] 
                  ...
                  if ( eddy_test == 1 ) 
                      ...
                      taux_inon    =  diag.taux(inon); %DL
                      taux_ave     =  tsnanmean(taux_inon); %DL
                      tauy_inon    =  diag.tauy(inon); %DL
                      tauy_ave     =  tsnanmean(tauy_inon); %DL

%DL                      spd_max     =  max(diag.spd(inon));
%DL % Added by Sanjiv (6/15/2020)
%DL                      sst_inon    =  diag.sst(inon);
%DL                      sst_ave     =  tsnanmean(sst_inon);
%DL                      sss_inon    =  diag.sss(inon);
%DL                      sss_ave     =  tsnanmean(sss_inon);
%DL                      utau_inon   =  diag.utau(inon);
%DL                      utau_ave    =  tsnanmean(utau_inon);
%DL                      vtau_inon   =  diag.vtau(inon);
%DL                      vtau_ave    =  tsnanmean(vtau_inon);
%DL                      qnet_inon   =  diag.qnet(inon);
%DL                      qnet_ave    =  tsnanmean(qnet_inon);
%DL                      sbl_inon    =  diag.sbl(inon);
%DL                      sbl_ave     =  tsnanmean(sbl_inon);
%%
%%
%DL                      vort_inon   =  diag.vort(inon) ;
%DL                      vort_ave    =  tsnanmean(vort_inon) ;
%DL                      if ( strcmp(etype,'cyc') == 1 )
%DL                          vort_max = max(diag.vort(in)) ; 
%DL                      else
%DL                          vort_max = min(diag.vort(in)) ; 
%DL                      end
                      neddy = neddy + 1 ;
                      ...
                      eddy.tauxave(neddy) = taux_ave   ; %DL
                      eddy.tauyave(neddy) = tauy_ave   ; %DL

%DL % Added by Sanjiv
%DL                      eddy.sstave(neddy) = sst_ave   ;
%DL                      eddy.sssave(neddy) = sss_ave   ;
%DL                      eddy.utau(  neddy)= utau_ave   ;
%DL                      eddy.vtau(  neddy)= vtau_ave   ;
%DL                      eddy.qnet(  neddy)= qnet_ave   ;
%DL                      eddy.sbl(   neddy)=  sbl_ave   ;
%%
                      eddy.cont(neddy)  = cont_vals(icv) ;
                      eddy.perr(neddy)  = errkm     ;
                      eddy.area(neddy)  = Carea     ;          % 14
%DL                      eddy.vort(neddy)  = vort_ave  ;
%DL                      eddy.vortmax(neddy)= vort_max  ;
%DL                      eddy.swirlmax(neddy)= spd_max  ;
