...
%-----------------
     ...
     if ( need_der )
        ...
        udx  = 2 * prog.u ./ ( grid.pm(:,1:isz-1) + grid.pm(:,2:isz) ) ; % u * av dx  -> m^2/s
        vdx  = 2 * prog.v ./ ( grid.pm(1:jsz-1,:) + grid.pm(2:jsz,:) ) ; % v * av dx  -> m^2/s       vom
        udy  = 2 * prog.u ./ ( grid.pn(:,1:isz-1) + grid.pn(:,2:isz) ) ; % u * av dy  ->             uon
        vdy  = 2 * prog.v ./ ( grid.pn(1:jsz-1,:) + grid.pn(2:jsz,:) ) ; % v * av dy  -> m^2/s
     ...
     if ( sum(strcmp(vnames,'spd')) == 1 )
         diag.spd  = ( u2rho_2d(prog.u).^2 + v2rho_2d(prog.v).^2 ).^0.5 ;
     end
