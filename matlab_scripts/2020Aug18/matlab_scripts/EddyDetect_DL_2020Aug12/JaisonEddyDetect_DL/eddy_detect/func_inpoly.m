function [cn, on] = func_inpoly ( tmp1, tmp2, tmp3, tmp4 )

%
%  MODIFY this documentation. Now, the function takes either 2 or 4 arguments.
%        if it is two  arg1 = p, as in default inpoly.m
%                      arg2 = node, as in default inpoly.m
%        if it is four arg1 = 2D-Matrix of x values (p)
%                      arg2 = 2D-Matrix of y values (p)
%                      arg3 = 1D-vector of x points (node)
%                      arg4 = 1D-vector of y points (node)
%    tolerance is fixed, and it is assumed that the node goes in continuous order.
%    
%    output will be on the same grid as that of input
%
...
%------------------------------------------------------------------------
% Downloaded from : http://www.mathworks.com/matlabcentral/fileexchange/loadFile.do?objectId=10391&objectType=FILE
% Jaison, June/05/2008
%------------------------------------------------------------------------

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%function [cn, on] = inpoly ( tmp1, tmp2, tmp3, tmp4 )

% ERROR CHECKING

     shape = size (tmp1) ;  % store input variable shape
     if ( nargin < 2 || nargin == 3 )
          error ([mfilename ':argchk'], '\n FATAL (%s) : Number of inputs should be either 2 or 4.\n', mfilename)
     end      

     if ( nargin == 4 ) 
          do_reshape = 1 ;
          if ( ~isvector(tmp1) && ~isvector(tmp2) )
             p = [tmp1(:) tmp2(:)] ;
          else 
             error ([mfilename ':argchk'], '\n FATAL (%s) : Arg1 & Arg2 should be 2D-Matrices, when narg=4.\n', mfilename)
          end
          tmp33 = tmp3 ; tmp44 = tmp4 ;
          if ( isvector(tmp3) && isvector(tmp4) )
             if ( size(tmp3,1) == 1 ) ;  tmp33 = (tmp3)' ; end 
             if ( size(tmp4,1) == 1 ) ;  tmp44 = (tmp4)' ; end 
             node = [tmp33 tmp44] ;
          else 
             error ([mfilename ':argchk'], '\n FATAL (%s) : Arg3 & Arg4 should be 1D-Vectors, when narg=4.\n', mfilename)
          end
          clear tmp3 tmp4 tmp33 tmp44 ;
     else
          do_reshape = 0 ;
          p = tmp1 ; node = tmp2 ;
     end
...
% reshape output variable to that of input
