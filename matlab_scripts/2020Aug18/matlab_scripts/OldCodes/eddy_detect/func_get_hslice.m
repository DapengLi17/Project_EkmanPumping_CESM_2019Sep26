...
%--------------
     ...
     ncr = netcdf ( filein, 'read' );
        ...
        hslice.taux = squeeze( ncr{'taux'}(tindx,:,:) ) .* grid.mask  ; % it is a 2D var
        hslice.tauy = squeeze( ncr{'tauy'}(tindx,:,:) ) .* grid.mask  ; % it is a 2D var
...
%          hslice.taux = squsqueeze( ncr{'taux'}(tindx,:,:) ) ) .* grid.mask  ; % it is a 2D var
%          hslice.tauy = squeeze( squeeze( ncr{'tauy'}(tindx,:,:) ) ) .* grid.mask  ; % it is a 2D var
