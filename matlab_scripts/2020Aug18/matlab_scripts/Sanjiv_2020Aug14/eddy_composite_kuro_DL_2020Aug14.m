...
%DL stats_dir ='/scratch/user/sanjiv/Matlab-code/RCESM/eddy_detect/output/diag/stats/nc/GoM_3km/';
stats_dir ='../data_after_manipulation/';  %DL

% grid_dir='/scratch/user/sanjiv/RCESM_output/kuro03_20031015_20040330_run08/fulldomain_historyfiles/';
%DL grid_dir='/scratch/user/sanjiv/RCESM_output/GOM_ocn3km_atm9km/';
grid_dir='../data_after_manipulation/'; %DL

% romsgrid    = strcat(directory,'kuro03_grid_N050_wrfmask.nc');

%DL coordfile = strcat(grid_dir,'grid_3hr.nc'); %DL Sanjiv includes ocean_time in the grid file and delete some unnecessary variables. The majority is the same as the origianl ROMS grid file kuro03_grid_N050_wrfmask.nc. 

%DL grid_struc=get_gridpar(coordfile);
...
grid_struc = func_init_grid(romsgrid,reffile, 'basic') ; %DL, use basic mode (same as Jaison's eddy_tracker_sla.circ.m)
 pm    = grid_struc.pm';
 pn    = grid_struc.pn';
 lonr  = grid_struc.lon';
 latr  = grid_struc.lat';
 maskr = grid_struc.mask'; 

%DL ocean_time = grid_struc.ocean_time;
%DL ntimes=max(size(ocean_time)); 


%----READ IN GRID PARAMETERS---------
%DL Cs_r    = grid_struc.Cs_r;
%DL theta_s = grid_struc.theta_s;
%DL theta_b = grid_struc.theta_b;
%DL hc      = grid_struc.hc;
%DL vtrans  = grid_struc.vtrans;
%DL vstret  = grid_struc.vstret;

%DL s_rho   = grid_struc.s_rho;
%DL s_w     = grid_struc.s_w;  
%DL nz_rho  = grid_struc.nz_rho;
%DL nz_w    = grid_struc.nz_w; 

%DL bathy = grid_struc.bathy;
%DL pm    = grid_struc.pm;
%DL pn    = grid_struc.pn;
%DL lonr  = grid_struc.lonr;
%DL latr  = grid_struc.latr;
%DL maskr = grid_struc.maskr; 
...
%DL fid_end     =  30  ; 
 fid_start   =  1   ; % id for first file %DL
 fid_end     =  2  ;                      %DL
...
%DL flist=dir(strcat(directory,'cmpr*ocn.hi*')); 
 flist=dir([directory '*Monthly*']); %DL
 fno = fid_start : fincr : fid_end; 

 nfiles = length(fno);

%DL reffile = [ directory,'KE03.ocn.hi.2003-12-01_06:00:00.nc' ];
%DL ncr = netcdf ( reffile, 'read' ) ;
%DL ot  = ncr{'ocean_time'}(:) ;
%DL nsnap = numel(ot) ;
%DL ncr = close (ncr) ;

%DL cyc_file = [stats_dir,'fid15_30_cyc.nc'];
 cyc_file = [stats_dir,'fid001_002_cyc.nc']; %DL cyclonic file
% cyc_file = [stats_dir,'fid001_002_anti.nc']; %DL anticyclonic file

%DL input file for eddy composite(file generated by new_tracker*.m). 
%DL Run the codes twice for cyclonic and anti-cyclonic eddy composites

 cyc_fid = ncread( cyc_file,'fid' )'; 
 cyc_tid = ncread( cyc_file,'tid' )'; %DL read the time
...
%DL reddy_fac = 5; % GoM 
  reddy_fac = 4; % DL
%DL 5=3+2, 3xradius for eddy composite, 2xradius(extra space for rotation). You lose some space when rototing eddy.
% 4 is OK for reddy_fac: -2xraidus to 2xradius for eddy composite 

% Vector used for permuting 2D dimension order (to call u2rho3d, etc.)
%DL dimorder = [2 1]; %DL ROMS specifics for Jaison's routine to flip Lat and Lon
...
%DL  lon_left = 144;  %DL left-most lon in deg, only composite eddy located to the right of that lon
%DL  lon_left = 280;     %DL lon ranges from 275 to 340 for the Gulf Stream region
%DL  ileft=find(lonr(:,100)-lon_left > 0 ,1,'first'); iright = nx; %DL index for regional box
 ileft   = 1 ; iright = nx;

%  lat_top = 42;
%  jtop =find(latr(100,:)-lat_top <0,1,'last'); jbottom = 1; 
  jbottom = 1 ; jtop =   ny;  %DL index for the background regional box
%%
...
%DL     if (fno(ifile)==68 |fno(ifile)==69 |fno(ifile)==70 ) 
%DL       continue
%DL     end 

     fname = flist(fno(ifile)).name;
     nsnap = length(ncread([directory fname],'ocean_time')); 
...
%   cyc_fid_end   = find(cyc_fid==flist(ifile),1,'last');
   ...
   for isnap = 1:nsnap;
     ...
     %isnap  
     otime = ncread([directory fname],'ocean_time',isnap,1,1); %DL read indivial ROMS output files
%     otime = ocean_time( (fno(ifile)-1)*nsnap + isnap );
     ...
     ssh = squeeze(ncread( strcat(directory,fname),'zeta',[1 1 isnap],[nx ny 1],[1 1 1] ));            %DL ssh = double(ssh);  
%DL     sst = squeeze(ncread( strcat(directory,fname),'temp',[1 1 nz_rho isnap],[nx ny 1 1],[1 1 1 1] )); sst = double(sst);  

%DL     qnet = squeeze(ncread( strcat(directory,fname),'shflux'  ,[1 1 isnap],[nx ny 1],[1 1 1] )); qnet = double(qnet);  
%DL     qlat = squeeze(ncread( strcat(directory,fname),'latent'  ,[1 1 isnap],[nx ny 1],[1 1 1] )); qlat = double(qlat);  
%DL     qsen = squeeze(ncread( strcat(directory,fname),'sensible',[1 1 isnap],[nx ny 1],[1 1 1] )); qsen = double(qsen);  
%DL     hsbl = squeeze(ncread( strcat(directory,fname),'Hsbl'    ,[1 1 isnap],[nx ny 1],[1 1 1] )); hsbl = double(hsbl);  

%DL     utau = squeeze(ncread( strcat(directory,fname),'sustr',[1 1 isnap],[nx-1 ny 1],[1 1 1] ));
%DL     vtau = squeeze(ncread( strcat(directory,fname),'svstr',[1 1 isnap],[nx ny-1 1],[1 1 1] ));  
%DL     utau = permute(u2rho_2d( permute(utau,dimorder) ), dimorder); %DL u2rho_2d: u to rho grids, 2d means for 2d array
%DL     vtau = permute(v2rho_2d( permute(vtau,dimorder) ), dimorder); 

     taux = squeeze(ncread( strcat(directory,fname),'taux',[1 1 isnap],[nx ny 1],[1 1 1] ));  %DL
     tauy = squeeze(ncread( strcat(directory,fname),'tauy',[1 1 isnap],[nx ny 1],[1 1 1] ));  %DL
...
%DL     tau = (utau.^2+vtau.^2).^0.5;

%DL     taucurl=get_windcurl(rho2u_2d(permute(utau,dimorder)),rho2v_2d(permute(vtau,dimorder)),pm,pn,maskr);
%DL rho2u_2d: rho grid to u grid, 2d is 2d variables. get_windcurl, rho2u_2d are functions. 
%DL needed to be changed for POP. taucurl on cell ceter.

%DL below for eddy composites, from actual model outputs

%DL     dutau_dx = (utau(2:end,:)-utau(1:end-1,:)).*...
%DL          0.5.*(pm(2:end,:)+pm(1:end-1,:));
%DL     dvtau_dy = (vtau(:,2:end)-vtau(:,1:end-1)).*...
%DL          0.5.*(pn(:,2:end)+pn(:,1:end-1));
%DL     dutau_dx = permute( u2rho_2d(permute(dutau_dx,dimorder)), dimorder);
%DL     dvtau_dy = permute( v2rho_2d(permute(dvtau_dy,dimorder)), dimorder);
%DL     taudiv = dutau_dx + dvtau_dy; 

%DL     dsst_dx = (sst(2:end,:)-sst(1:end-1,:)).*...
%DL          0.5.*(pm(2:end,:)+pm(1:end-1,:));
%DL     dsst_dy = (sst(:,2:end)-sst(:,1:end-1)).*...
%DL          0.5.*(pn(:,2:end)+pn(:,1:end-1));
%DL     dsst_dx = permute( u2rho_2d(permute(dsst_dx,dimorder)), dimorder);
%DL     dsst_dy = permute( v2rho_2d(permute(dsst_dy,dimorder)), dimorder);
%DL     grad_sst = (dsst_dx.^2 + dsst_dy.^2).^0.5; 

%
%DL --- compute Ekman pumping velocity --- {DL 2020Aug18}
...
% LOOP THROUGH THE EDDIES AT EACH TIMESTEP
     for ieddy=1:cyc_neddy
       ...
       eddy_rad = cyc_rkm(ieddy); %DL box for eddy composite
       max_rad = reddy_fac*eddy_rad; 

% Use (i,j) on ROMS grid nearest to eddy center %DL
...
%DL throw away eddies near edges
...
%
        for i=i0:nx %i0 eddy center, nx is the total edge, DL
           dist = sw_dist([latr(i,j0) lat0],[lonr(i,j0) lon0],'km');
           if (dist/eddy_rad > reddy_fac)
             break
           end 
        end
        imax = i-1; imin = i0-(imax-i0); %imax the right-most values of i within the eddy, DL
        for j=j0:ny
        ...
        jmax = j-1; jmin = j0-(jmax-j0); % index for 5x5 radius square box DL

% Discard eddies such that a point reddy_fac*R from its center falls on land or is outside the domain

        if ( imin <= ileft | imax >= iright | jmin <= jbottom | jmax >= jtop | ...   %DL discard eddy near the coasts (eddy on land)
             ~isempty( find(isnan(maskr(imin:imax,jmin:jmax))==1) ) )
          continue
        end 

        xl_subset = sw_dist([latr(imin,jmin) latr(imax,jmin)],[lonr(imin,jmin) lonr(imax,jmin)],'km'); %DL length of the eddy box, one num in km
        yl_subset = sw_dist([latr(imin,jmin) latr(imin,jmax)],[lonr(imin,jmin) lonr(imin,jmax)],'km'); 

% Get means of variables over [imin:imax,jmin:jmax] % DL mean wind dir for rotations, space-mean
        utau_mean=nanmean(nanmean(utau(imin:imax,jmin:jmax))); 
        vtau_mean=nanmean(nanmean(vtau(imin:imax,jmin:jmax))); 

%
%DL --- rotate x and y to west to east dir ---
        ...
        xo = dist.*cos(arg); yo = dist.*sin(arg); %xo is original x refereed to eddy center, DL, change from eddy to eddy, xo and yo are not rotated
%DL ------------------------------------------
...
%        err_center(ieddy) = sw_dist([cyc_lat(ieddy) latr(cyc_i(ieddy), cyc_j(ieddy))],[cyc_lon(ieddy) lonr(cyc_i(ieddy),cyc_j(ieddy))],'km');
       ...
       [X,Y]=meshgrid(size(xrot,1),size(xrot,2)); %DL, X: 0 0.1R, 0.2R, ... 5R, important, X Y are roatated with wind dir
...
%DL sanjiv said you can comment the three lines below: 
...
%DL, imin, imax are the box for eddy, xgrid is the final grids for plots 
...
% The interpolation step
       ...
       composite.ssh = composite.ssh + ssh_grid; %DL build composites
