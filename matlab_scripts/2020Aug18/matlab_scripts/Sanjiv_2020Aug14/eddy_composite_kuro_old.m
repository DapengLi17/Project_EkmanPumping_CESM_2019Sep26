...
% flist = fid_start : fincr : fid_end; 
 ...
 cyc_file = [stats_dir,'fid15_30_cyc.nc']; %input files for eddy composite, DL, file generated by new trackers. run the codes twice for cyclonic and anti-cyclonic eddy

 cyc_fid = ncread( cyc_file,'fid' )'; 
 cyc_tid = ncread( cyc_file,'tid' )'; % read the time. DL

% Initialize eddy count
 composite.ok  = 0;
 composite.nok = 0;  % not all eddy are good for composites, e.g. close to the edge %DL
 composite.tot = 0;

% Multiplier of eddy radius that sets the extent of the domain where fields 
% are saved
% reddy_fac = 6; % Kuroshio
 reddy_fac = 5; % GoM %DL 5=3+2, 3 times radius for eddy composite, 2 extra space for rotations, when you rotate you lose some space, 5 is ok. use 4 for reddy_fac, -2 to 2 radius  

% Vector used for permuting 2D dimension order (to call u2rho3d, etc.)
 dimorder = [2 1]; %ROMS specifics for Jaison's routine to flip Lat and Lon
...
% Kuroshio domain subset for eddy statistics. Ignore eddies outside this region (to avoid land, shallow depths)
  lon_left = 144;  % left most lon in deg DL
  ileft=find(lonr(:,100)-lon_left > 0 ,1,'first'); iright = nx; %DL index for regional box
% ileft   = 1 ; iright = nx;

%  lat_top = 42;
%  jtop =find(latr(100,:)-lat_top <0,1,'last'); jbottom = 1; 
  jbottom = 1 ; jtop =   ny;    %DL index for regional box
%%
...
% Final scaled grid to which values are interpolated
  [xgrid,ygrid] = meshgrid( -(reddy_fac-1):0.1:(reddy_fac-1), -(reddy_fac-1):0.1:(reddy_fac-1) );   %DL -4 to 4 radius for interpolation
...
%   cyc_fid_end   = find(cyc_fid==flist(ifile),1,'last');
   ...
   for isnap = 1:nsnap;
     ...
     %isnap  
     otime = ncread([directory fname],'ocean_time',isnap,1,1); %DL indivial ROMS output files
%     otime = ocean_time( (fno(ifile)-1)*nsnap + isnap );
     ...
     utau = permute(u2rho_2d( permute(utau,dimorder) ), dimorder); %DL u2rho_2d: u to rho grids
     ...
     taucurl=get_windcurl(rho2u_2d(permute(utau,dimorder)),rho2v_2d(permute(vtau,dimorder)),pm,pn,maskr); %DL rho grid to u grid, 2d is 2d variables. get_windcurl, rho2u_2d are functions. need to be changed for POP. taucurl on cell ceter.

% compute Ekman W vel. DL

%DL below for composites, from actual model outputs
...
%
     cyc_lon = ncread(cyc_file,'x'  ,cyc_start,cyc_neddy,1); % cyc_file is from the eddy static file created from the tracker
...
% LOOP THROUGH THE EDDIES AT EACH TIMESTEP
     for ieddy=1:cyc_neddy
       ...
       eddy_rad = cyc_rkm(ieddy); %DL eddy composite box
       max_rad = reddy_fac*eddy_rad; %DL box 
